...
class DeepLSTM(Model):
  ...
  def prepare_model(self, data_dir, dataset_name, vocab_size):
    ...
    # print '\n\n\n\nembed_inputs shape check', embed_inputs.get_shape()
    ...
    # states shape: batch_size, output_size( depth * size )
    # expected output shape max_length, batch_size, output_size
  ...
  def train(self, sess, vocab_size, epoch=25, learning_rate=0.0002,
    ...
    for epoch_idx in xrange(epoch):
      data_loader = load_dataset(data_dir, dataset_name, vocab_size, test=True)
